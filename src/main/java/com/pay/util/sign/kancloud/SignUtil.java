package com.pay.util.sign.kancloud;
...
public class SignUtil {
	...
	public static <T extends BaseUrlEntity> String getSign(T t)throws Exception{
		...
		for(Field f : fields){
			if(f.isAnnotationPresent(PayRequestParamLabel.class)){
				...
				if(paramLabel.isSign()){
				    ...
				    String labelName = StringUtils.isNotBlank(paramLabel.name()) ? paramLabel.name() : f.getName();
					//不为空才需要签名
					if(StringUtils.isNotBlank(value)){
						if(paramLabel.sortIndex() == 0){
							values.add(labelName + "=" + value+"&");
						}else{
		...
		if(StringUtils.isNotBlank(key)){
			sign = sign + "apikey="+key;
		}
		logger.info("需要加密的字符串：" + sign);
		String signStr = Md5.encode(sign).toUpperCase();
		logger.info("加密完全的字符串:" + signStr);
		return signStr;
	}
